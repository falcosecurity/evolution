...
```
k8s-using-daemonset$ kubectl create -f ./falco-account.yaml
...
```
k8s-using-daemonset$ kubectl create -f ./falco-service.yaml
...
```
k8s-using-daemonset$ mkdir -p falco-config
k8s-using-daemonset$ cp ./falco.yaml ./falco-config/
k8s-using-daemonset$ cp ./rules/falco_rules.* ./falco-config/
k8s-using-daemonset$ cp ./rules/k8s_audit_rules.yaml ./falco-config/
```
...
```
k8s-using-daemonset$ kubectl create configmap falco-config --from-file=./falco-config
...
```
k8s-using-daemonset$ kubectl create -f ./falco-daemonset-configmap.yaml
